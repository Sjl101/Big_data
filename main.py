...
df = spark.read.csv('Assignment 1/nuclear_plants_small_dataset.csv', inferSchema=True, header=True).na.fill(0)
...
TrueN = dtpredict.where((col('prediction')=='0') & (col('indexedLabel')=='0')).count()
FalseN = dtpredict.where((col('prediction')=='1') & (col('indexedLabel')=='0')).count()
TrueABN = dtpredict.where((col('prediction')=='1') & (col('indexedLabel')=='1')).count()
FalseABN = dtpredict.where((col('prediction')=='0') & (col('indexedLabel')=='1')).count()

statlist = ["min","max","mean","median","mode","lower","upper"]
Sen = (TrueN + FalseABN)
Sensitivity = (TrueN / Sen)
Spe = (TrueABN + FalseN)
Specificity = (TrueABN / Spe)
matrix = spark.createDataFrame([("Normal", TrueN, FalseN),("Abnormal", FalseABN, TrueABN)],['Status','True',"False"])
